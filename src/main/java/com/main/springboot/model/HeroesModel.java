...
public class HeroesModel extends Pattern {
    public String printSpecTable(String comics_json) {
        return printByPattern(comics_json, patternSpecTable( "<thead><tr><td>id</td><td>name</td><td>image</td><td>description</td><td>comics</td></tr></thead>"));
    }
    ...
    public String printByPattern(String string_json, ArrayList<String> pattern) {
        ...
        while (parser.getValByProp(string_json, Integer.toString(k)).length()>0)
        {
            ...
            output += pattern.get(2);
            output += "<a href=\"./hero?id="+id_hero+"\" target=\"_blank\">"+parser.getValByProp(hero, "name")+"</a>";
            output += pattern.get(3);
            ...
            output += pattern.get(2);
            ItemsModel comics = new ItemsModel();
            output += comics.printComma(parser.getValByProp(hero, "comics"));
            output += pattern.get(3);
